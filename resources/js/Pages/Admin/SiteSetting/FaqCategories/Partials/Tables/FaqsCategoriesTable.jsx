...
export default function FaqsCategoriesTable({ categories, onEdit }) {
  const { t } = useTrans();

  // Individual category actions
  const deleteCategory = (category) => {
    if (confirm(t('confirm_delete_category'))) {
      router.delete(route('admin.faq-categories.destroy', category.id), {
        preserveScroll: true,
      });
    }
  };
  ...
  const handleBulkDelete = async (ids) => {
    router.delete(route('admin.faq-categories.bulk.delete'), {
      data: { ids },
      preserveState: true,
      preserveScroll: true,
    });
